  ...
  getAll: async (req, res) => {
    try {
      ...
      const postsWithTotalLikes = posts.map((item) => {
        ...
        const isLiked = postInfo.likes.findIndex((like) => like.userId === 1) > -1;
        const post = { ...postInfo, totalLikes: postInfo.likes.length, isLiked };
  ...
  create: async (req, res) => {
    ...
    try {
      ...
      if (categories.length < 1)
        return sendResponse(res, 400, "Você tem que escolher pelo menos 1 categoria.");
      ...
      const categoriesExist = await Category.findAll({ where: { categoryId: categories } });

      if (categoriesExist.length < 1)
        return sendResponse(res, 404, "Oops... Categoria(s) não encontrada(s).");
